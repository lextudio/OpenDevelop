...
**A commit is not the only trigger.** `Get-PinnedGitVersionProperties` in `dist.ps1` documents the
...
`dist.ps1` solves it by reading back the values the host publish already wrote and passing them as
...
- Observed after one full solution build: `src/Main/SharpDevelop/bin` kept a good copy (so the app
  ...
  testable regardless.
- `dist.ps1`'s `Sync-LibreWpfTransportRuntime` overlays `runtimes/<rid>/lib/net10.0` over the publish
  output, which is what makes a RID-specific publish correct even when `lib/` holds an
  architecture-stamped `DirectWriteForwarder`. **A plain `dotnet build` gets no such overlay**, so a
  non-RID development build is only ever correct for the packaging machine's architecture.
