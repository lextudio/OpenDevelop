	...
	public class FindBaseClassesOrMembers : AbstractMenuCommand
	{
		...
		async Task RunAsync()
		{
			...
			var model = new ContextActionsPopupViewModel {
				Title = MenuService.ConvertLabel(StringParser.Parse("${res:SharpDevelop.Refactoring.BaseClassesOf}", new StringTagPair("Name", result.Subject))),
		...
		static async Task<SymbolHierarchyResult> QueryAsync()
		{
			...
			return (await protocol.TypeHierarchySupertypesAsync(document, editor.Caret.Offset, CancellationToken.None)).Value;
		}
	}
}
