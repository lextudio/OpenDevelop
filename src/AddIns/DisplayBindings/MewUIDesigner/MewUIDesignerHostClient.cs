...
sealed class MewUIDesignerHostClient : IDesignHostClient
{
	...
	Connection connection; DesignerDocumentSnapshot? recoverySnapshot; DesignerSessionState? recoveredState; bool disposed; public string DocumentId { get; } = Guid.NewGuid().ToString("N");
	...
	MewUIDesignerHostClient(Connection connection) { this.connection = connection; connection.HostExited += OnHostExited; lock (clientsGate) clients.Add(this); }
	...
	public Task<DesignerSessionState> OpenAsync(DesignerDocumentSnapshot s, CancellationToken t = default) { Stamp(s); recoverySnapshot = s; return connection.OpenAsync(s, t); }
	public Task<DesignerSessionState> UpdateAsync(DesignerDocumentSnapshot s, CancellationToken t = default) { Stamp(s); recoverySnapshot = s; return connection.UpdateAsync(s, t); }
	public Task<DesignerEditSet> FlushAsync(long v, CancellationToken t = default) => connection.FlushAsync(DocumentId, v, t);
	public Task<DesignerSessionState> SetPropertyAsync(long v, string id, string p, string value, CancellationToken t = default) => TrackAsync(connection.SetPropertyAsync(DocumentId, v, id, p, value, t), t);
	public Task<DesignerSessionState> AddElementAsync(long v, string parent, DesignerToolboxItemInfo item, string name, double x, double y, CancellationToken t = default) => TrackAsync(connection.AddElementAsync(DocumentId, v, parent, item, name, x, y, t), t);
	public Task<DesignerSessionState> DeleteElementsAsync(long v, string[] ids, CancellationToken t = default) => TrackAsync(connection.DeleteElementsAsync(DocumentId, v, ids, t), t);
	public Task<DesignerSessionState> RenameAsync(long v, string id, string name, CancellationToken t = default) => TrackAsync(connection.RenameAsync(DocumentId, v, id, name, t), t);
	...
	async Task CaptureAsync(long version, CancellationToken token) { if (recoverySnapshot == null) return; var edit = await connection.FlushAsync(DocumentId, version, token).ConfigureAwait(false); recoverySnapshot.Version = version; recoverySnapshot.Files.Clear(); foreach (var file in edit.Files) recoverySnapshot.Files.Add(file); }
