...
`DragOver` delivery is invisible from the outside; add temporary `Console.WriteLine` tracing (gated
on an env var) in `CreateComponentTool.designPanel_DragOver` and
`CreateComponentTool.AddItemsWithCustomSize`, launch with `OD_TEST_MODE=1`, and drive the gesture
over DevFlow. Notes that cost time:
