...
One-time converter: feed an existing `*.Designer.cs` through the CURRENT
`MewUIDocumentEditor.Parse()` node tree and emit `.mxaml`. Shipped as a DevFlow action
(`od.mewui-designer.convert-to-mxaml`) so fixtures migrate inside the running app.
